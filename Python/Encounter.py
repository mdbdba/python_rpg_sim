...
class Encounter(object):
    ...
    def __init__(self,
        ...
        for position, Hero in enumerate(heroes):
            self.add_to_initiative_array(Hero, 'Heroes', self.h_cnt, position)

        self.o_cnt = len(opponents)
        for position, Opponent in enumerate(opponents):
            self.add_to_initiative_array(Opponent, 'Opponents', self.o_cnt,
                                         position)
    ...
    def get_array_index(self, x: int, y: int) -> int:
        """
        Return the location in the field_size list of an X, Y coordinate.
        """
        ret_val: int = ((x * (self.field_size)) + y)
        return ret_val

    def sector_is_unoccupied(self, x: int, y: int) -> bool:
        list_index: int = self.get_array_index(x, y)
        ret_val: bool = not self.field_map[list_index].occupied
        print(f"Is [{x}][{y}] field_map[{list_index}] available: {ret_val}")
        if (not ret_val):
    ...
    def get_target_distance_array(self, my_x, my_y, target_name):
        ...
        for x in range(self.field_size * self.field_size):
            if ((self.field_map[x].occupied) and
               self.field_map[x].occupied_by == target_name):
                a, b = self.get_grid_position(x)
                dist = self.calculate_distance(my_x, my_y, a, b)
                dist_list.append([dist, a, b, self.field_map[x].occupied_by,
                                 self.field_map[x].occupied_by_index])
    ...
    def add_to_initiative_array(self, src_list: list, source_array_name: str, array_size: int,
                                source_array_position: int):
        pc = src_list.Check('Perception', 'Normal', 10)
        ini = src_list.rollForInitiative('Normal')
        field_max = self.field_size - 1

        if source_array_name == 'Heroes':
            MapLocX = 1
        else:
            MapLocX = field_max

        if source_array_position == 0:
            MapLocY = int(field_max/2)
        elif source_array_position % 2 == 0:
            MapLocY = (int(field_max/2) + int(source_array_position / 2)
                       + (source_array_position % 2 > 0))
        else:
            MapLocY = (int(field_max/2) - int((source_array_position / 2)
                                              + (source_array_position % 2 > 0)))

        pos = self.get_array_index(MapLocX, MapLocY)
        self.field_map[pos].occupy_sector(source_array_name,
                                          source_array_position)

        self.initiative.append([source_array_name, source_array_position,
                                pc, ini,
                                MapLocX, MapLocY])
    ...
    def turn(self, initiative_ind, waiting_for):
                if self.initiative[initiative_ind][0] == "Heroes":
                    curActive = self.Heroes[self.initiative[initiative_ind][1]]
                ...
                if (not self.initiative[initiative_ind][2]):
                    ...
                    self.initiative[initiative_ind][2] = (
            ...
            if (cur_active.combat_preference == 'Melee'):
                ...
                while (mvmt):
                    ...
                    if (mvmt):
                        ...
                        move_success = False
                        for i in range(3):
                            if (self.sector_is_unoccupied(tlX[i], tlY[i])):
